...
@MainActor
final class ConsultationTimer: ObservableObject {
     ...
     */
    init(lengthInMinutes: Int = 0, attendees: [Consultation.Attendee] = []) {
     ...
     */
    func reset(lengthInMinutes: Int, attendees: [Consultation.Attendee]) {
        self.lengthInMinutes = lengthInMinutes
        self.speakers = attendees.speakers
        secondsRemaining = lengthInSeconds
        activeSpeaker = speakerText
...
extension Array<Consultation.Attendee> {
    var speakers: [ConsultationTimer.Speaker] {
        if isEmpty {
            return [ConsultationTimer.Speaker(name: "Speaker 1", isCompleted: false)]
        } else {
            return map { ConsultationTimer.Speaker(name: $0.name, isCompleted: false) }
        }
    }
}
